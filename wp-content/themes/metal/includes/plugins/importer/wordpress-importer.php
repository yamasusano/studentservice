...
<?php
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( $response_code != '200' ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', sprintf( esc_html__('Remote server returned error response %1$d %2$s', 'wordpress-importer'), esc_html( $response_code ), get_status_header_desc( $response_code ) ) );
		}
...
if ( !function_exists( 'zozo_wp_update_nav_menu_item' ) ) {
	...
	function zozo_wp_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
		...
		if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) )
			return new WP_Error('update_nav_menu_item_failed', esc_html__('The given object ID is not that of a menu item.', 'wordpress-importer'));
